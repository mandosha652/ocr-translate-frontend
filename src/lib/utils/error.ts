...
function extractErrorString(error: AxiosError): string {
...
function mapErrorToFriendly(raw: string, fallback: string): string {
...
export function getErrorMessage(
  ...
  if (error instanceof AxiosError) {
    ...
    const friendly = mapErrorToFriendly(raw, fallback);
    if (friendly !== fallback) return friendly;
